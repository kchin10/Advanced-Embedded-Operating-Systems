...
//	int io_hole = IOPHYSMEM/KERNBASE = 96;
	for (i = 0; i < npages; i++) {
		if(i == 0 || ((i >= 96) && (i <= init_allocated_mem)) || (i == PGNUM(MPENTRY_PADDR))){
			pages[i].pp_ref = 1;
		} else {
			pages[i].pp_ref = 0;
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	//
	// Your code here:
	void *start = ROUNDDOWN((void *)pa,PGSIZE);
	uint32_t rounded_size = ROUNDUP(pa + size,PGSIZE) - (uint32_t)start;
	if(base + rounded_size >= MMIOLIM) {
		panic("mmio_map_region -> request memory %x above MMIOLIM (%x)",pa ,MMIOLIM);
	}
	boot_map_region(kern_pgdir,base,rounded_size,(uintptr_t)start,PTE_P | PTE_W | PTE_PCD | PTE_PWT);
	cprintf("IO mapped at %x for size %u ending at %x\n",base,rounded_size,base + rounded_size);
	base += rounded_size;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for (;start<va + len;start += PGSIZE){
		pte = pgdir_walk(env->env_pgdir,start,0);
		if ((uint32_t)start >= ULIM || !pte || !(*pte & PTE_P) || ((*pte | perm) == 0) ){
