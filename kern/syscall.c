...
sys_env_set_status(envid_t envid, int status)
{
	...
	if (envid2env(envid,&target_env,true) != 0){
		return -E_BAD_ENV;
	}
...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if (((perm & ~(PTE_SYSCALL)) != 0) || ((perm & (PTE_U | PTE_P)) != (PTE_P | PTE_U))) {
		return -E_INVAL;
	}
	...
	if (page_insert(target_env->env_pgdir,pg,va,perm) != 0) {
		page_free(pg);
		return -E_NO_MEM;
	}
	return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	pte_t *srcpte;
	if ((srcpg = page_lookup(srcenv->env_pgdir,srcva,&srcpte)) == NULL) {
		return -E_INVAL;
	}
	...
	page_insert(dstenv->env_pgdir,srcpg,dstva,perm);
	return 0;
}
